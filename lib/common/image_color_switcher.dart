...
class IconAssetColorSwitcher extends StatelessWidget {
  const IconAssetColorSwitcher(this.imagePath,
  {
...
//      required this.colorMap, 
      super.key, this.fit, this.filterQuality, this.width, this.height,
      this.assetWidth, this.assetHeight
...
class _AssetColorSwitcherState extends State<AssetColorSwitcher> {
  ...
  @override
  void initState() {
    // AssetImage(widget.imagePath).resolve(ImageConfiguration()).addListener(
    //     ImageStreamListener((info, _) => info.image.toByteData(format: ui.ImageByteFormat.png).then(
...
//            final switchedImage = await switchColor(buffer);
          }
...
//        if (r == c.key.red && g == c.key.green && b == c.key.blue) {
  ...
  //      }
    ...
    ui.decodeImageFromPixels(pixels, image!.width, image!.height, ui.PixelFormat.rgba8888, 
      (data) => 
//        this.image = data;
            setState(() {
              this.image=data;
