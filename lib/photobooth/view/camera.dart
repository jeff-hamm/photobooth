...
import 'package:io_photobooth/common/camera_service_value.dart';
...
class _CameraState extends State<Camera> {
  ...
  //     if(svc.value.cameraId != cameraId) {
  //       _preview = null;
  //     }
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder(
      valueListenable: context.watch<CameraService>(),
      builder: (BuildContext context, CameraServiceValue state, _) {
        ...
        if (state.cameraError != null) {
          return widget.error(context,
              state.cameraError!.description ?? state.cameraError!.code);
        }
        return widget.preview(context, const SizedBox());
      },
    );
  }
}
